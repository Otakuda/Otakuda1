...
function Search_lot_number()/*  search lot_number by database and start_date end end_data*/
{
   ...
   alert("開始日期與結束日期不能為空");
	...
	else
	{
        ...
        url='../index.php/Report/Search_lot_number' + '/' +database.value + '/' + start.value + '/' + end.value;
               ...
               xmlhttp.onreadystatechange = function()
                    {
                        if (xmlhttp.readyState == 4 && xmlhttp.status == 200)
                      {
                        ...
                        document.getElementById('Col_Header').value=message;
                        //var Chart_col_header = JSON.parse(message);
                       
                      }
                    };
         var url="../index.php/Report/Get_Chart_Header";
         xmlhttp.open("post", url, true);
         xmlhttp.send();
  }

...
Class backmodel extends CI_Model
{
	...
	function count_table($where,$table)//計算幾筆分頁
	{
	...
	function find_data_all($flowcard,$where,$table)
	{
		$this->db->select('no,FLOWCARD_NO,ETI_NO,SEQNO,QTY,LABEL_CNT,WASHQTY,OUTQTY,box,machine,workorder,Iniwashtime,cust_seqno,special_code');
		...
		$col=array('序號','流程卡號','ETI_NO','序號','數量','標籤數','已清洗','已出貨','箱號','機型','工單','清洗次數','客戶編號','特殊編碼');
		$count=count($col);
		for($j=0;$j<$count;$j++)
		{
